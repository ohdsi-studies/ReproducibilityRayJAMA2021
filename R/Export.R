...
#
# This file is part of ReproducibilityRayJAMA2021
#
...
exportAnalyses <- function(outputFolder, exportFolder) {
  ...
  cmAnalysisListFile <- system.file("settings",
                                    "cmAnalysisList.json",
                                    package = "ReproducibilityRayJAMA2021")
...
exportExposures <- function(outputFolder, exportFolder) {
  ...
  pathToCsv <- system.file("settings", "TcosOfInterest.csv", package = "ReproducibilityRayJAMA2021")
  tcosOfInterest <- read.csv(pathToCsv, stringsAsFactors = FALSE)
  pathToCsv <- system.file("Cohorts.csv", package = "ReproducibilityRayJAMA2021")
  ...
  createExposureRow <- function(exposureId) {
    ...
    cohortFileName <- system.file("cohorts", paste0(exposureId, ".json"), package = "ReproducibilityRayJAMA2021")
...
exportOutcomes <- function(outputFolder, exportFolder) {
  ...
  createOutcomeRow <- function(outcomeId) {
    ...
    cohortFileName <- system.file("cohorts", paste0(outcomeId, ".json"), package = "ReproducibilityRayJAMA2021")
    definition <- readChar(cohortFileName, file.info(cohortFileName)$size)
  ...
  pathToCsv <- system.file("settings", "NegativeControls.csv", package = "ReproducibilityRayJAMA2021")
...
exportMetadata <- function(outputFolder,
  ...
  database <- tibble(databaseId = databaseId,
                     ...
                     studyPackageVersion = utils::packageVersion("ReproducibilityRayJAMA2021"),
                     isMetaAnalysis = 0)
...
exportDiagnostics <- function(outputFolder,
  ...
  cluster <- ParallelLogger::makeCluster(min(4, maxCores))
  ParallelLogger::clusterRequire(cluster, "ReproducibilityRayJAMA2021")
